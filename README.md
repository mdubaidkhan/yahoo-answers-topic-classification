...
|---|---|---|
| RQ1: Classifier performance at scale | LinearSVC best overall; confusion concentrated in Edu&Ref ↔ Sci&Math as predicted by vocabulary overlap; TF-IDF representation reaches a ceiling by 753K samples | Macro-F1 = **0.733** (vs. 0.687 probe at 10K) |
| RQ2: Value of more text | Config C > B > A confirmed; A→B gain (+0.051) far exceeds B→C (+0.009); gain is heterogeneous and directly predicted by per-category missingness — Biz&Fin gained least (+0.032), Fam&Rel gained most (+0.113) | McNemar p ≈ 4.1×10⁻¹²⁴ |
...
The dataset files are too large to commit to this repo (~500 MB). Download `train.csv` and `test.csv` from the Kaggle link above and place them at `data/train.csv` and `data/test.csv`. See [`data/README_data.md`](./data/README_data.md) for full instructions.
...
2. Download `train.csv` and `test.csv` from Kaggle (link above) and upload them to your Colab session via **Files → Upload**
...
│
├── checkpoints/
│   ├── checkpoint_1.ipynb       # CP1: Dataset selection & initial EDA
│   └── checkpoint_2.ipynb       # CP2: Research questions & pilot experiments
│
└── data/
...
4. McInnes, L., Healy, J., & Melville, J. (2018). UMAP: Uniform Manifold Approximation and Projection. *JOSS*. arXiv:1802.03426.# yahoo-answers-topic-classification